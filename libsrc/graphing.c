...
DLL_EXPORT_SYM gdouble rlib_graph(rlib *r, struct rlib_part *part, struct rlib_report *report, gdouble left_margin_offset, gdouble *top_margin_offset) {
	...
	gdouble y_value;
	gdouble stacked_y_value_max[2] = {0,0};
	gdouble stacked_y_value_min[2] = {0,0};
	gdouble y_value_try_max[2];
	gdouble y_value_try_min[2];
	gdouble col_sum = 0;
	gdouble tmi;
	gdouble running_col_sum = 0;
	gdouble y_min[2] = {0,0};
	gdouble y_max[2] = {0,0};
	...
	if (rlib_execute_as_double(r, graph->width_code, &tmp))
		graph_width = tmp;
	if (rlib_execute_as_double(r, graph->height_code, &tmp))
		graph_height = tmp;
	...
	if (!INPUT(r, r->current_result)->isdone(INPUT(r, r->current_result), r->results[r->current_result]->result)) {
		while (1) {
			...
			row_sum = g_realloc(row_sum, (row_count+1) * sizeof(gdouble));
			row_sum[row_count] = 0;
			for (list = graph->plots; list != NULL; list = g_slist_next(list)) {
				...
				if (rlib_execute_as_string(r, plot->axis_code, axis, MAXSTRLEN)) {
					if (strcmp(axis, "x") == 0 && !is_pie_graph(graph_type)) {
						...
						if (!disabled && rlib_execute_as_double(r, plot->field_code, &y_value)) {
							side = RLIB_SIDE_LEFT;
	...
	last_row_values = g_malloc(i * sizeof(gdouble));
	last_row_height = g_malloc(i * sizeof(gdouble));
